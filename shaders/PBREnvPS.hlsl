// PBR Pixel Shader with Normal Mapping, Shadow Mapping, and Environment Reflections
...
float3 CalculateEnvironmentReflection(float3 N, float3 V, float3 F0, float roughnessValue)
{
    // Calculate reflection vector
    float3 R = reflect(-V, N);

    // Sample environment map
    // For now, we use LOD 0 (no roughness-based mip selection since we don't have prefiltered maps yet)
    float3 envColor = environmentMap.SampleLevel(linearSampler, R, roughnessValue * 4.0).rgb;

    // Calculate fresnel for environment
    float NdotV = max(dot(N, V), 0.0);
    float3 F = FresnelSchlickRoughness(NdotV, F0, roughnessValue);

    // Approximate specular IBL (simplified without prefiltered maps)
    // Reduce reflection intensity based on roughness
    float specularStrength = 1.0 - roughnessValue;
    specularStrength *= specularStrength;  // Square for better falloff
...
float4 main(PSInput input) : SV_TARGET
{
    ...
    // Environment reflection (more visible on metallic and smooth surfaces)
    float3 envReflection = CalculateEnvironmentReflection(N, V, F0, roughness);

    // Ambient lighting (includes environment reflection)
    float3 ambient = float3(0.03, 0.03, 0.03) * albedoColor * ao;

    // Add environment reflection
    // Metallic surfaces reflect more of the environment
    ambient += envReflection * lerp(0.2, 1.0, metallic);

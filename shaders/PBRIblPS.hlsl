// PBR Pixel Shader with Image-Based Lighting (IBL)
...
// Image-Based Lighting (IBL) ambient calculation
float3 CalculateIBLAmbient(float3 N, float3 V, float3 F0, float3 albedoColor, float metallicValue, float roughnessValue, float aoValue)
{
    float NdotV = max(dot(N, V), 0.0);
    ...
    // Final ambient with AO
    float3 ambient = (kD * diffuse + specular) * aoValue;
...
float4 main(PSInput input) : SV_TARGET
{
    ...
    float3 ambient = CalculateIBLAmbient(N, V, F0, albedoColor, metallic, roughness, ao);
